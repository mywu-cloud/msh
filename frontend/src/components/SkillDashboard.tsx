...
function PriceCell({ price }: { price?: PriceInfo | null }) {
  if (!price || !price.close) return <><td className="text-center px-2 py-2 text-xs text-slate-600 whitespace-nowrap" style={{minWidth:'90px'}}>—</td><td className="text-center px-2 py-2 text-xs text-slate-600 whitespace-nowrap" style={{minWidth:'64px'}}>—</td><td className="text-center px-2 py-2 text-xs text-slate-600 whitespace-nowrap" style={{minWidth:'90px'}}>—</td></>
  const isPos = price.change > 0
  const isNeg = price.change < 0
  const cls = isPos ? 'text-red-600' : isNeg ? 'text-green-600' : 'text-slate-700'
  return <>
    <td className={`text-center px-2 py-2 text-xs font-medium whitespace-nowrap ${cls}`} style={{minWidth:'90px'}}>{price.close.toFixed(2)}</td>
    <td className={`text-center px-2 py-2 text-xs font-medium whitespace-nowrap ${cls}`} style={{minWidth:'64px'}}>{isPos ? '+' : ''}{price.change.toFixed(2)}</td>
    <td className={`text-center px-2 py-2 text-xs font-medium whitespace-nowrap ${cls}`} style={{minWidth:'90px'}}>{isPos ? '+' : ''}{price.change_pct.toFixed(2)}%</td>
  </>
}
        ...
        <thead>
          <tr className="bg-slate-50 border-b border-slate-200">
            ...
            {hasPrice && <>
              ...
              <th className={thClass('price_change_pct')} style={{minWidth:'90px'}} onClick={() => onSort('price_change_pct')}>漲跌幅<SortIcon col="price_change_pct" sortKey={sortKey} sortDir={sortDir} /></th>
            </>}
          </tr>
        </thead>
