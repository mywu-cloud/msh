...
async function handleBigHolderChanges(request: Request, env: Env): Promise<Response> {
  ...
  try {
    const datesResult = await env.DB.prepare(`SELECT DISTINCT date FROM holder_distribution WHERE date >= '20260605' ORDER BY date DESC LIMIT ${weeks + 1}`).all(); // 20260605 前為系統上線前回補資料，分級不完整（mid/small 恆為0），暫時排除
...
async function handleStockDetail(request: Request, env: Env, stockCode: string): Promise<Response> {
  ...
  if (cached) return new Response(cached, { headers: { ...CORS_HEADERS, "X-Cache": "HIT" } });
  try {
    const datesResult = await env.DB.prepare("SELECT DISTINCT date FROM holder_distribution WHERE date >= '20260605' ORDER BY date DESC LIMIT 13").all(); // 20260605 前為系統上線前回補資料，分級不完整，暫時排除
    const allDates = (datesResult.results || []).map((r: Record<string, unknown>) => r.date as string).sort();
